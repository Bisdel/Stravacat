...
@Configuration
@EnableMethodSecurity(prePostEnabled = false) // TODO Repasser à true Activer les annotations PrePost pour les accès
public class SecurityConfig {
	...
	public SecurityFilterChain filterChain(HttpSecurity http, JwtHeaderAuthorizationFilter jwtFilter) throws Exception {
		...
		http.authorizeHttpRequests(authorize -> {
			...
			// authorize.requestMatchers("/api/animal/inscri").permitAll(); // Autorisé à
			// tout le monde
			// authorize.requestMatchers("/api/utilisateur/**").permitAll(); // Autorisé à
			// tout le monde

			// Les accès seront configurés via les annotations PrePost
			// authorize.requestMatchers("/api/fournisseur/**").hasRole("ADMIN"); //
			// Autotisé aux utilisateurs "admin"

			// authorize.requestMatchers("/**").authenticated(); // Autorisé aux
