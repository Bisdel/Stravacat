...
public class AbonneApiController { // Injection des dépendances IAbonnesRepository et IVilleRepository
	...
	public List<AbonnesResponse> findAll() {
		...
		List<AbonnesResponse> result = this.repoAbonne.findAll()
				.stream()
	...
	@PostMapping
	public AbonnesResponse add(@Valid @RequestBody AbonnesRequest abonnesRequest,
	...
	@PutMapping("/{id}")
	public Abonnes add(@PathVariable int id,
			@Valid @RequestBody AbonnesRequest abonnesRequest,
			BindingResult result) {
		// Vérifier s'il y a des erreurs de validation dans le BindingResult
		if (result.hasErrors()) {
		...
		Abonnes abonnes = this.repoAbonne
				.findById(id).orElseThrow(AbonnesNotFoundException::new);
